...
const StartupScreen = ({status, ffmpegDownload, microphoneAccess, setupCompleted}) => {
  return (
    <div
      style={{display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', background: '#3399ee'}}>
      <div className="startup-block">
        ...
        <p style={{marginBottom: '1em'}}>
          Avant de commencer, Moiki Vocalizer doit <b>récupérer <em>ffmpeg</em></b> (pour encoder et mixer les sons)
          et <b>avoir
        ...
        {status.ffmpegReady ? (
          <div>Librairie ffmpeg <Label color="green">OK</Label></div>
            ...
            <div>Erreur lors de l'installation de ffmpeg <Label color="red">FAILED</Label></div>
          ) : (
            <Fragment>
              {(status.ffmpegProgress >= 0) ? (
                <Segment style={{margin: '1.8em', width: 300}}>
                  ...
                  <div style={{
                    ...
                    <div style={{
                      height: '100%',
                      width: (status.ffmpegProgress * 100) + '%',
                      background: '#2185D0',
                      display: 'block'
                    }}/>
                  </div>
        ...
        <Divider style={{width: '100%'}}/>
        <Button size="big" primary disabled={!status.microphoneReady || !status.ffmpegReady}
                onClick={setupCompleted}>Démarrer</Button>
      </div>
    </div>
  )
}
